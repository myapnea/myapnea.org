...
class ResearchTopicsControllerTest < ActionController::TestCase
  ...
  #   login(@no_votes_user)
  #   get :first_topics, params: { read_intro: 1 }
  #   assert_response :success
  # end
  ...
  test 'should endorse any approved research topic as experienced user' do
    ...
    assert_difference 'Vote.count' do
      post :vote, params: { research_topic_id: @rt2.id, "endorse_#{research_topics(:rt2).id}" => 1 }
    end
  ...
  test 'should add comment when voting for topic' do
    ...
    assert_difference 'Post.count' do
      post :vote, params: { research_topic_id: @rt2.id, "endorse_#{research_topics(:rt2).id}" => 1, "comment_#{research_topics(:rt2).id}" => comment }, format: 'js'
    end
  ...
  #   end
  #   assert_difference 'Vote.count' do
  #     post :vote, params: { research_topic_id: rt.id, "endorse_#{rt.id}" => 1 }
  #   end
  # end
  ...
  test 'should add remote votes for research topics' do
    ...
    assert_difference 'Vote.count' do
      post :remote_vote, params: { research_topic_id: rt.id, "endorse_#{rt.id}" => 1 }, format: 'js'
    end
    assert_difference 'Vote.count' do
      post :remote_vote, params: { research_topic_id: rt2.id, "endorse_#{rt2.id}" => 0 }, format: 'js'
    end
  end
  ...
  test 'should not add remote vote with unexpected input for research topic' do
    ...
    rt = ResearchTopic.where(category: 'seeded').first
    assert_no_difference 'Vote.count' do
      post :remote_vote, params: { research_topic_id: rt.id, "endorse_#{rt.id}" => 3 }, format: 'js'
    end
  ...
  test 'should change vote' do
    ...
    assert_response :success
    assert_no_difference 'Vote.count' do
      post :change_vote, params: { research_topic_id: @rt.id }
    end
  end
  ...
  test 'should oppose any approved research topic as experienced user' do
    ...
    assert_equal 0.0, @rt2.endorsement
    assert_not_nil @rt2
    get :show, params: { id: @rt2 }
    assert_response :success
    assert_difference 'Vote.count' do
      post :vote, params: { research_topic_id: @rt2.id, "endorse_#{research_topics(:rt2).id}" => 0 }
    end
  ...
  test 'should not accept votes with no endorsement or opposition chosen' do
    login(@experienced_user)
    assert_no_difference 'Vote.count' do
      post :vote, params: { research_topic_id: research_topics(:rt2).id }
    end
  end
end
