class AccountController < ApplicationController
  ...
  def privacy_policy
    if params[:privacy_policy_read]
      ...
      if current_user.ready_for_research?
        redirect_to (session[:return_to].present? ? session.delete(:return_to) : root_path), notice: "You have now signed the consent and are ready to participate in research. You can opt out any time by visiting your user account settings."
      else
  ...
  def consent
    if params[:consent_read]
      ...
      if current_user.ready_for_research?
        redirect_to (session[:return_to].present? ? session.delete(:return_to) : root_path), notice: "You have now signed the consent and are ready to participate in research."
      else
