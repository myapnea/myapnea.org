class Answer < ActiveRecord::Base
  ...
  def update_response_value!(val)
    ...
    val = {} if val.nil? or not val.kind_of?(Hash)
  ...
  def update_response_value_from_api!(val)
    ...
    if template.parent_answer_template_id.present?
      ...
      parent_response_value = answer_session.answer_values(question, parent_template)
      set_completion_state([parent_response_value.include?(template.parent_answer_option_value) ? answer_values.present? : true])
    else
      set_completion_state([answer_values.present?])
    end
